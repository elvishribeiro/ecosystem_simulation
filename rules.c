...
/*find an empty slot*/
coord_t empty_neighbour(object_t **eco, config_t conf, coord_t p) {
	srand(time(NULL));
	coord_t possible[4];
	int possible_n = 0;
	...
	if (possible_n > 0)
		if (possible_n > 1)
			return possible[(conf.GEN + p.x +p.y) % possible_n];
		else
			return possible[0];
	else 
...
coord_t rabbit_move (object_t **eco, object_t **next_eco, config_t conf, coord_t p) {
	coord_t new = empty_neighbour(eco, conf, p);										
	if (new.x >= 0 && new.y >= 0){
